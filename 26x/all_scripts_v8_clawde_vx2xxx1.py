...
# ---------- パス設定 ----------------------------------------------------------
video_path        = r"C:\Users\s1280\Desktop\SHRP2rawdata\3\new\3_Combined_FaceSizeCentered_bottom_left.mp4"
csv_path          = r"C:\Users\s1280\Desktop\SHRP2rawdata\3\new\3_acc_speed_filled_complete.csv"
...
output_video_path = video_path.replace(".mp4", "_combined_speed_yolo_vx5.mp4")
output_csv_path   = video_path.replace(".mp4", "_trajectories_combined5.csv")
...
try:
    while cap.isOpened():
        ...
        for t in tracks:
            ...
            # ★修正：X座標とY座標の両方を中心座標として浮動小数点計算
            cx_float = x0 + bw_c / 2.0  # X座標の中心
            cy_float = y0 + bh_c / 2.0  # ★Y座標も中心座標として計算
            ...
            # === トラック履歴を整数座標で保存（描画用） ===
            track_history.setdefault(tid, []).append((cx_int, cy_int))

            # === トラック履歴を線で描画（整数座標使用） ===
            for k in range(1, len(track_history[tid])):
                pt1 = track_history[tid][k - 1]  # これは(int, int)
                pt2 = track_history[tid][k]  # これも(int, int)
            ...
            trajectories.append({
                "frame": frame_idx,
                "id": tid,
                "x": float(cx_float),  # ★浮動小数点座標
                "y": float(cy_float),  # ★浮動小数点座標
                "area": bw_c * bh_c,
                "class": combined_label,
                "speed": float(v_now),
                "risky": int(cv2.pointPolygonTest(poly_pts_img.astype(np.int32), (cx_int, cy_int), False) >= 0),
                "intent": combined_label,
            })

            # === ポリゴン内判定（整数座標使用） ===
            in_risk_zone = cv2.pointPolygonTest(poly_pts_img.astype(np.int32), (cx_int, cy_int), False) >= 0
            ...
            if risk_zone_counter[tid] >= 2:
                if tid not in llava_status_map or not llava_status_map[tid]["active"]:
                    ...
                    driving_facts = {
                        ...
                        "v_now": v_now,
                        "intent": combined_label,
                        "cx": cx_float,  # ★ここも浮動小数点
                        "cy": cy_float,  # ★ここも浮動小数点
                        "area": bw_c * bh_c
                    }
    ...
    try:
        if trajectories:
            ...
            # ★修正: データ型を明示的に指定
            df_traj["x"] = df_traj["x"].astype("float64")
            df_traj["y"] = df_traj["y"].astype("float64")
            df_traj["speed"] = df_traj["speed"].astype("float64")

            # ★修正: 小数点以下の桁数を指定してCSV保存
            df_traj.to_csv(output_csv_path, index=False, float_format="%.3f")

            print("✅ CSVファイルを保存しました（浮動小数点3桁）")
    except Exception as e:
        print(f"⚠️ CSV保存でエラー: {e}")
