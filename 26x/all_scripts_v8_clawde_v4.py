...
import textwrap
# ---------- パス設定 ----------------------------------------------------------
video_path        = r"C:\Users\s1280\Desktop\SHRP2rawdata\4\new\4_Combined_FaceSizeCentered_bottom_left.mp4"
csv_path          = r"C:\Users\s1280\Desktop\SHRP2rawdata\4\original\4_acc_speed_filled_complete.csv"
...
# ワーカー処理（バックグラウンドでLLaVA推論）
def risk_worker():
    while True:
        try:
            tid, frame_idx, img_path, facts = risk_queue.get(timeout=1)
        except queue.Empty:
            continue

        try:
            result = assess_risk_from_image(img_path, facts)
            ...
            print(f"✅ [Async LLaVA] tid={tid} frame={frame_idx} risk={result.get('risk_probability')}")
...
# デュアル構成 → 2スレッド起動
for _ in range(2):
    threading.Thread(target=risk_worker, daemon=True).start()
...
try:
    while cap.isOpened():
        ...
        for i,t in enumerate(tracks):
            ...
            # === カウンタ初期化（1台目で未登録なら） ===
            if 'risk_zone_counter' not in globals():
                from collections import defaultdict
            ...
            # === 8フレーム以上連続で入っている場合にのみ生成AIをジョブ登録 ===
            if risk_zone_counter[tid] >= 8:
                # まだLLaVAが動いてないならジョブ投入
                if tid not in llava_status_map or not llava_status_map[tid].get("active", False):
                    risk_frame_path = f"4_risk_frame_{frame_idx}.jpg"
                    ...
                    # 🚨 ここでは実行せず、キューに渡すだけ
                    risk_queue.put((tid, frame_idx, risk_frame_path, driving_facts))

                    # 結果が返るまで「active=True」にしておく（表示用）
                    llava_status_map[tid] = {"active": True}

            else:
                # リスクゾーンから出たら表示をリセット（必要なら）
        ...
        if representative is not None:
            ...
            llava_line1 = f"LLaVA: w={weather} f={fog} r={rain} rd={road}"
        ...
        # 60pxの帯にテキスト描画
        llava_vis = np.zeros((60, 360, 3), dtype=np.uint8)
        llava_vis = draw_text_clean(llava_vis, llava_line1, (5, 6), font_size=7)
        if llava_line2: llava_vis = draw_text_clean(llava_vis, llava_line2, (5, 20), font_size=7)
        if llava_line3: llava_vis = draw_text_clean(llava_vis, llava_line3, (5, 34), font_size=7)
        if llava_line4: llava_vis = draw_text_clean(llava_vis, llava_line4, (5, 48), font_size=7)
