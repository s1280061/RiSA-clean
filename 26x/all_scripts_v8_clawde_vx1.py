...
# ---------- モデル ------------------------------------------------------------
det_model   = YOLO("yolov8m.pt").to("cuda")        # 車・トラック
...
try:
        ...
        for i,t in enumerate(tracks):
            ...
            if risk_zone_counter[tid] >= 1:
                if tid not in llava_status_map or not llava_status_map[tid]["active"]:
                    risk_frame_path = f"risk_frame_{global_frame_idx}.jpg"
                    ...
                    for tr in trajectories:
                        if tr["frame"] == frame_idx:  # このフレームのものだけ
                            detected_vehicle_list.append({
                                "id": tr["id"],
                                "type": tr["class"],  # car / truck
                                "intent": tr["intent"],  # straight / left / right / brake
                    ...
                    json_record = {
                        ...
                        "ego_vehicle": {
                            ...
                            "brake_pressed": (intent == "brake"),
                            "left_blinker": (intent == "left"),
                            "right_blinker": (intent == "right"),
                            "lane_change": (intent in ["left", "right"]),
                            "risk_zone": True
                        },
    ...
    # === JSON 保存 ===
    output_json_path = video_path.replace(".mp4", "_covla.json")
    if json_records:
