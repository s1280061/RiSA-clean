...
try:
    while cap.isOpened():
        ...
        if representative is not None:
            ...
            # 文字数制限でreasonを短縮（画面幅に応じて調整）
            max_chars_per_line = 120  # 5pxフォントで画面幅に収まる文字数
            if len(reason_text) > max_chars_per_line:
                llava_line2 = reason_text[:max_chars_per_line - 3] + "..."
            else:
        ...
        if frame_idx % 10 == 0:
            ...
            print(f"[{frame_idx:6d}/{total_frames}] {pct:5.1f}% "
                  ...
                  f"elapsed:{elapsed:6.1}s")

        frame_idx += 1
except KeyboardInterrupt:
    print("⏹️ Ctrl+Cで中断されました。現在の進捗まで保存します...")
    print(f"📊 処理済みフレーム数: {frame_idx}/{total_frames}")
    ...
    try:
        if 'cap' in locals() and cap is not None:
            cap.release()
            print("✅ 入力動画を正常にクローズしました")
    except Exception as e:
        print(f"⚠️ 入力動画のクローズ中にエラー: {e}")

    # 2. 動画出力のリリース（最重要！）
    try:
        if 'out' in locals() and out is not None:
            out.release()
            print("✅ 出力動画を正常にクローズしました")

            # ファイルサイズをチェック
            if os.path.exists(output_video_path):
                file_size = os.path.getsize(output_video_path)
                print(f"📁 出力動画サイズ: {file_size:,} bytes")
                if file_size < 1000:
                    print("⚠️ 動画ファイルが小さすぎます。破損している可能性があります")
            else:
                print("❌ 出力動画ファイルが見つかりません")

    except Exception as e:
        print(f"❌ 出力動画のクローズ中にエラー: {e}")

    # 3. OpenCVウィンドウのクリーンアップ
    try:
        cv2.destroyAllWindows()
        print("✅ OpenCVウィンドウをクリーンアップしました")
    except Exception as e:
        print(f"⚠️ OpenCVウィンドウのクリーンアップ中にエラー: {e}")

    # 4. CSVファイルの保存
    try:
        if 'trajectories' in locals() and len(trajectories) > 0:
            df_trajectories = pd.DataFrame(trajectories)
            df_trajectories.to_csv(output_csv_path, index=False)
            print(f"✅ 軌跡データを保存しました: {len(trajectories)}行")
            print(f"📄 CSV: {output_csv_path}")
        else:
            print("⚠️ 保存する軌跡データがありません")
    except Exception as e:
    ...
    try:
        ...
        fps_processed = frame_idx / total_elapsed if total_elapsed > 0 else 0
        print("\n" + "=" * 50)
        print("📊 処理完了サマリー")
        print("=" * 50)
        print(f"⏱️  総処理時間: {total_elapsed:.1f}秒")
        print(f"🎬 処理フレーム数: {frame_idx:,}/{total_frames:,}")
        print(f"⚡ 処理速度: {fps_processed:.1f} FPS")
        print(f"🎥 出力動画: {output_video_path}")
        print(f"📊 出力CSV: {output_csv_path}")

        # 完了率の表示
        completion_rate = (frame_idx / total_frames * 100) if total_frames > 0 else 0
        print(f"✅ 完了率: {completion_rate:.1f}%")

        if completion_rate < 100:
            print("⚠️  処理が完了していません。部分的な結果が保存されました。")

    except Exception as e:
        print(f"⚠️ サマリー表示中にエラー: {e}")
...
def check_video_integrity(video_path):
    ...
    try:
        ...
        print(f"   ⚡ FPS: {fps:.2f}")
        print(f"   ⏱️  再生時間: {frame_count / fps:.1f}秒")
...
# finally ブロックの最後に動画チェックを追加
if os.path.exists(output_video_path):
    print("\n🔍 出力動画の健全性をチェック中...")
    check_video_integrity(output_video_path)