...
# NOTE: CSV 名に合わせて既定は p20/f45 に揃える
H_PAST, H_FUT = 30, 45
...
def ade_fde_pixels(pred, gt, W=W_PX, H=H_PX):
    # 正規化座標を px に戻してから距離
    pred_px = pred.clone()
    gt_px   = gt.clone()
...
def main():
    ...
    parser.add_argument("--img_w", type=float, default=DEF_IMG_W)  # ← ここを W_PX ではなく定数に
    parser.add_argument("--img_h", type=float, default=DEF_IMG_H)  # ← 同上
