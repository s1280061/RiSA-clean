...
Modes:
  ...
  - turn_lowconf  : 景色条件を外し、YOLOウィンカーの低信頼度優先で --total 件を単純抽出
...
Priority when mode=turn_lowconf:
  - sort by (turn_conf asc, brake_conf asc, image_path asc), dedup by image_path
  - take top --total
...
def gather_items(root, bases=("3","4","5","6"), require_env=True):
    ...
    require_env=True  のとき:
      stage1_env の YES フラグが少なくとも1つ必要（balanced 用）
    require_env=False のとき:
      stage1_env が空でも通す（turn_lowconf 用）
    """
    items = []
    for base in bases:
        ...
        for ctx_path in ctx_files:
            ...
            for fr in frames:
                ...
                items.append({
                    "image_path": img,
                    "envs": env_yes,  # turn_lowconf では空の可能性あり
                    "turn": turn,
                    "turn_conf": t_conf,
                    "brake": brake,
                    "brake_conf": b_conf,
...
def select_by_low_turn_conf(items, total=50):
    """
    turn_conf の昇順（低→高）を最優先。
    tie-breaker: brake_conf 昇順 → image_path 昇順。
    重複は image_path で除去。
    """
    items_sorted = sorted(
        items,
        key=lambda d: (d["turn_conf"], d["brake_conf"], d["image_path"])
    )
    ...
    with open(manifest_path, "w", newline="", encoding="utf-8") as f:
        ...
        for i, it in enumerate(selected, start=1):
            ...
            if not os.path.isfile(src_img):
                # 画像が存在しない場合はスキップ（manifest には書かない）
                continue
...
def main():
    ...
    # 新規: モードと件数
    ap.add_argument("--mode", choices=["balanced", "turn_lowconf"], default="balanced",
                    help="balanced: 旧ロジック（配分充足） / turn_lowconf: ウィンカー低信頼度優先")
    ap.add_argument("--total", type=int, default=50,
                    help="mode=turn_lowconf の抽出件数")
    ...
    # モードに応じて gather のフィルタ（環境必須か）を切替
    ...
    if args.mode == "balanced":
        ...
        # target_total は env/turn/brake の合計
    else:
        # 低ウィンカー信頼度優先・単純抽出
