...
import json
import torch
from trajectory_predictor_module import load_trained_model, predict_trajectory

model_path = r"C:\Users\s1280\Desktop\SHRP2rawdata\3\LSTMdataset_new_1\lstm_trajectory_model.pth"
device = 'cuda' if torch.cuda.is_available() else 'cpu'
trajectory_model = load_trained_model(model_path, device)

from collections import defaultdict
buffer = defaultdict(list)  # トラッキングIDごとの履歴保存用
...
turn_buffer = defaultdict(list)
brake_buffer = defaultdict(list)
trajectory_buffer = defaultdict(list)  # ← ここが重要！

print(f"[FRAME] shape: w={w}, h={h}")
...
try:
    while cap.isOpened():
        ...
        for t in tracks:
            ...
            if len(trajectory_buffer[tid]) > 10:
                trajectory_buffer[tid].pop(0)

            # === デバッグ: 履歴の長さ確認 ===
            print(f"[TID {tid}] 履歴長: {len(trajectory_buffer[tid])}")

            # === 軌跡予測が可能な場合（10ステップ履歴が溜まった） ===
            if len(trajectory_buffer[tid]) == 10:
                print(f"✅ 軌跡予測を実行: tid={tid}, 履歴={trajectory_buffer[tid]}")
                input_np = np.array(trajectory_buffer[tid])  # shape: (10, 2)
                pred_future = predict_trajectory(trajectory_model, input_np, device, frame_size=(w, h))

                print(f"🧠 予測された軌跡（{len(pred_future)}ステップ）: {pred_future}")

                # === 軌跡の可視化（赤ドット） ===
                for px, py in pred_future:
                    if 0 <= px < w and 0 <= py < h:
                        cv2.circle(frame, (int(px), int(py)), 2, (0, 0, 255), -1)
                    else:
                        print(f"⚠️ 描画範囲外: ({px}, {py})")
