...
# ---------- BEV 変換用座標 ----------------------------------------------------
src  = np.load(os.path.join(base_path, "src_points_3_forward_2nd.npy"))
dst  = np.load(os.path.join(base_path, "dst_points_3_forward_2nd.npy"))
...
try:
    while cap.isOpened():
        ...
        if left_pts and right_pts:
            ...
            # --- 停止距離計算 ---
            stop_m_raw = brake_distance(df_speed, frame_idx)
            stop_m = max(stop_m_raw, 10.0)  # 最低10m確保
            ...
            # === ★ カーブ追従のために複数サンプル ===
            num_samples = 20  # サンプル数を増やすと滑らか
            y_vals = np.linspace(y_bot, y_top, num_samples)

            # 左右レーンの曲線座標
            left_curve = np.array([[np.polyval(l_coef, y), y] for y in y_vals])
            right_curve = np.array([[np.polyval(r_coef, y), y] for y in y_vals])

            # ポリゴンは左カーブ→右カーブ（逆順）で閉じる
            poly_bev = np.vstack([
                left_curve,
                right_curve[::-1]
            ]).astype(np.float32)
